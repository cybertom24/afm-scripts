...
for i = 1:length(dataset)
    for j = 1:length(normalization_types)
        for k = 1:length(mean_removal)
            type = normalization_types{j};
            remove_mean = mean_removal{k};

            index = (i-1) * (length(normalization_types) * length(mean_removal)) + (j - 1) * length(mean_removal) + k;
            X = dataset(i).X;
            shifts = dataset(i).shifts;
            points_per_map = dataset(i).points_per_map;
            
            if isequal(remove_mean, 'meanremv')
                for map = 1:(size(X, 1) / points_per_map)
                    start = (map - 1) * points_per_map + 1;
                    stop = map * points_per_map;
                    avg = mean(X(start:stop, :), 1);
            ...
            for f = fieldnames(dataset(i))'
                dataset_normalized(index).(f{1}) = dataset(i).(f{1});
            end
            % Aggiorna quelli presenti
            dataset_normalized(index).X = X;
            dataset_normalized(index).name = sprintf('%s-%s-%s', dataset(i).name, type, remove_mean);
        end
    end
end

clear d_X d_name type i j k l index remove_mean x X Y shifts spect start stop map f avg points_per_map
...
for i = 1:length(pca_results)
    figure;
    tiledlayout("flow");
    sgtitle(pca_results(i).name);

    nexttile;
    hold on;
    grid on;
    plot(pca_results(i).shifts, pca_results(i).coeff(:, 1:3));
    ...
    bar(pca_results(i).explained);
    xlabel('PC index');
    ylabel('Explained variance [%]');
    set(gca, 'YScale', 'log');
    
    Y = pca_results(i).score(:, 1:pca_results(i).max_PC);
    Y_iso = Y(1:pca_results(i).tot_iso, :);
    Y_acm = Y((pca_results(i).tot_iso + 1):end, :);
    ...
    grid on;
    hold on;
    plot(pca_results(i).shifts, pca_results(i).pcx_coeff);
    title('PCx loadings');
    xlabel('shift [cm^{-1}]');
    ylabel('coeff [a.u.]');
    ...
    violinplot(pca_results(i).pcx_scores(1:pca_results(i).tot_iso), 'DensityDirection', 'negative', 'FaceColor', [0.07 0.44 0.75], 'DisplayName', 'H-CO');
    violinplot(pca_results(i).pcx_scores((pca_results(i).tot_iso + 1):end), 'DensityDirection', 'positive', 'FaceColor', [0.87 0.33 0.00], 'DisplayName', 'D-CO');
    xlabel('PCx');
    ylabel('PC score [a.u.]');
    legend show;
    ...
    for i_map = 1:(length(pca_results(i).pcx_scores) / pca_results(i).points_per_map)
        if ((i_map-1) * pca_results(i).points_per_map < pca_results(i).tot_iso)
            violinplot(i_map * ones(1, pca_results(i).points_per_map), pca_results(i).pcx_scores(((i_map-1) * pca_results(i).points_per_map + 1):(i_map * pca_results(i).points_per_map)), 'FaceColor', [0.07 0.44 0.75]);
        else
            violinplot(i_map * ones(1, pca_results(i).points_per_map), pca_results(i).pcx_scores(((i_map-1) * pca_results(i).points_per_map + 1):(i_map * pca_results(i).points_per_map)), 'FaceColor', [0.87 0.33 0.00]);
        end
    end
    xlabel('Map index');
    ylabel('PC score [a.u.]');
    ...
    nexttile;
    axis off;
    L_string = sprintf('%.2f ', pca_results(i).L);
    text(0, 1, ...
...
% Fallo più volte e plotta i risultati
n_repetitions = 100;
...
for i_map = 1:n_maps
    ...
    % Salva i risultati
    err_rate_list(i_map, 1) = err_rate;
    err_rate_list(i_map, 2) = err_iso / (length(pcx_scores_excluded_iso) + length(pcx_scores_excluded_acm));
    err_rate_list(i_map, 3) = err_acm / (length(pcx_scores_excluded_iso) + length(pcx_scores_excluded_acm));
...
mean_err = mean(err_rate_list(:, 1));
mean_err_iso = mean(err_rate_list(:, 2));
mean_err_acm = mean(err_rate_list(:, 3));
...
nexttile;
title('Success rate, false negatives and false positives');
grid on;
hold on;
xlabel('Map removed');
ylabel('Percentage [%]');
b = bar(1:n_maps, rates * 100, 'stacked');
b(1).FaceAlpha = 0.65;
b(2).FaceAlpha = 0.65;
b(3).FaceAlpha = 0.65;
b(1).FaceColor = [0.23 0.67 0.20];
b(2).FaceColor = [0.87 0.33 0.00];
b(3).FaceColor = [0.07 0.44 0.75];
legend('Success rate', 'False negatives', 'False positives', 'Location', 'best');

nexttile;
axis off;
text(0, 1, ...
        sprintf('Name: %s\nPC used for LDA: %d\nN° of repetitions: %d\nAverage false negatives: %.2f%%\nAverage false positives: %.2f%%\nAverage success rate: %.2f%%\n', pca_results(i).name, pca_results(i).max_PC, n_maps, mean_err_acm * 100, mean_err_iso * 100, (1 - mean_err) * 100), ...
        'HorizontalAlignment', 'left', 'VerticalAlignment', 'top', 'FontSize', 12);
