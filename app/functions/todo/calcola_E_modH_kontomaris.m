...
function [E, Erid, u_E, u_Erid] = calcola_E_modH_kontomaris(z, d, k, R, v)
    ...
    % Fit model to data.
    [fitted_curve, gof, out] = fit( h, f, ft, opts );
    ...
    figure;
    scatter(h, f, 'DisplayName', 'curva FH');
    hold on;
    plot(h, f_fit, 'DisplayName', 'fit ah^b');
    xlabel('h [m]');
    ylabel('f [N]');
    title(['fit f(h) = a \cdot h^b dove b = ' num2str(b)]);
